...
export default {
  ...
  build_debug() {
    atom.notifications.addInfo('Build started. (Debug)', {})
  ...
  build_o2() {
    atom.notifications.addInfo('Build started. (Optimized)', {})
    this.build(['-O2'])
  },
  ...
  build(extraOptions) {
    ...
    tokens = tokens.concat(['-std=c++17', '-pthread', '-fdiagnostics-format=json', '-Wall'])
